"""Fact table loader operator for Redshift.

This operator loads data into fact audit tables
"""
...
class LoadFactOperator(BaseOperator):
    """Load data into a fact audit table in Redshift.
    
    Args:
        redshift_conn_id: Airflow connection ID for Redshift
        destination_table: Target fact audit table name
        sql_query: SQL query to extract source data
        partition_column: Column used for partition-based deletion
    """
    ...
    def execute(self, context):
        """Execute fact audit table load operation."""
        redshift = PostgresHook(postgres_conn_id=self.redshift_conn_id)

        # Truncate audit
        self.log.info(f'Truncating fact audit table audit.{self.destination_table}')
        truncate_sql = f"""
        ...
        # Insert data
        self.log.info(f'Loading data into fact audit table audit.{self.destination_table}')
        insert_sql = f"""
